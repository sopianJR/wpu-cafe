    ...
    const handleAddToCart = (type: string, id: string, name: string) => {
        ...
        if (type === 'increment') {
            ...
            if (itemIsInCart && itemIsInCart.quantity <= 1) {
                ...
                setCarts(
                    carts.map((item: ICart) => item.menuId === id ? {...item, quantity: item.quantity - 1} : item,
                    ),
                );
            }
        }
        ...
        <main className={styles.create}>
            <div className={styles.menu}>
                ...
                <div className={styles.filter}>
                    {filters.map((filter) => (
                        <Button 
                            type="button"
                            color={(!searchParams.get('category') && filter === 'All') ||
                                filter === searchParams.get('category') ? 'primary' : 'secondary'}
                            key={filter}
            ...
            <form className={styles.form} onSubmit={handleOrder}>
                ...
                <div className={styles.input}>
                    <Input id="name" label="Name" name="customerName" placeholder="Inser Name" required />
                    <Select id="table" label="Table Number" name="tableNumber" required options={tables} />
                </div>
                <div>
                    <div className={styles.header}>
